...
def refresh(widget):
	...
	widget.destroy()
"""
def createEntry(parent,cn,rw,stick):
	return Entry(parent).grid(column=cn,row=rw,sticky=stick)
"""
...
def createNode(Error=False):
	...
	def on_create():
		try:
			...
			createButton(main_app,"create Connection",8,1,'NEWS',createConnection)
			createButton(main_app,"save network",9,1,'NEWS',saveNetwork)
			createButton(main_app,"cancel",10,1,'NEWS',cmd=cancel)
		except ValueError:
			createNode(True)
...
def createConnection(Error=False):	
	...
	def on_create():
		try:
			...
			createButton(main_app,"create another Connection",8,1,'NEWS',createConnection)
			createButton(main_app,"save network",9,1,'NEWS',saveNetwork)
			createButton(main_app,"cancel",10,1,'NEWS',cmd=cancel)
		except ValueError:
